...
from app.routes.auth import USERS_DB, TOKENS_DB, OTP_STORE, hash_password, verify_password, create_access_token
...
def test_media_upload_endpoint():
    ...
    # Valid minimal 1x1 JPEG byte sequence
    jpeg_bytes = bytes([
    ...
    bad_files = {"file": ("test.exe", jpeg_bytes, "application/x-msdownload")}
    res_bad = client.post("/api/complaints/upload-media", files=bad_files, headers={"Authorization": f"Bearer {token}"})
